...
CREATE TABLE `broadcast` (
    ...
    `enabled`       TINYINT DEFAULT 1 COMMENT '是否启用: 0-禁用, 1-启用',
    `description`   VARCHAR(500) DEFAULT NULL COMMENT '描述',
    `voice_name`    VARCHAR(100) DEFAULT 'serena' COMMENT '语音角色',
    `voice_speed`   DECIMAL(3,1) DEFAULT 1.0 COMMENT '语速 0.5~2.0',
    `voice_volume`  DECIMAL(3,1) DEFAULT 80 COMMENT '音量 0~100',
    `voice_file_path` VARCHAR(500) DEFAULT NULL COMMENT '生成的语音文件路径',
    `create_time`   DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `update_time`   DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    PRIMARY KEY (`id`),
    KEY `idx_enabled` (`enabled`)
...
CREATE TABLE `broadcast_strategy` (
    ...
    `play_interval`     INT         DEFAULT 0 COMMENT '播放间隔（分钟），0表示持续，>0表示最小间隔',
    `description`       VARCHAR(500) DEFAULT NULL COMMENT '描述',
    `create_time`       DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `update_time`       DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    PRIMARY KEY (`id`),
